...
def insert_user():
    ...
    app_line = Label(frameRight, width=400, height=1, anchor=NW, font=('Ivy 10'), bg=co3, fg=co1)
    ...
    #E-mail
    l_email = Label(frameRight, text="Endereço de E-mail*", anchor=NW, font=('Ivy 10'), bg=co1, fg=co4)
    ...
    #Telefone
    l_phone = Label(frameRight, text="Número de Telefone*", anchor=NW, font=('Ivy 10'), bg=co1, fg=co4)
    ...
    b_save = Button(frameRight, image=img_save, compound=LEFT, width=100, anchor=NW, text="Salvar", bg=co1, fg=co4, overrelief=RIDGE, relief=GROOVE, font=('Ivy 11'))
    b_save.grid(row=7, column=1, sticky=NSEW, pady=5)

#Ver usuários
def see_users():
    app_ = Label(frameRight, text="Todos os Usuários do Banco de Dados", width=50, compound=LEFT, padx=5, pady=10, anchor=NW, relief=FLAT, font=('Verdana 12'), bg=co1, fg=co4)
    ...
    app_line.grid(row=1, column=0, columnspan=3, sticky=NSEW)
    data = get_user()
    list_header = ['ID', 'Nome', 'Sobrenome', 'Endereço', 'E-mail', 'Telefone']
    global tree
    ...
    h = [20, 80, 80, 120, 120, 120 ,76, 100]
    n= 0 

    for col in list_header:
        tree.heading(col, text=col, anchor='nw')
        # adjust the column's width to the header string
        tree.column(col, width=h[n], stretch=NO, anchor=hd[n])
        n += 1
